...
df_train = get_dataset('amazon_reviews_multi')
...
dataset_train = TensorDataset(input_ids_train_1, attention_masks_train_1,input_ids_train_2, attention_masks_train_2,input_ids_train_3, attention_masks_train_3,input_ids_train_4, attention_masks_train_4,input_ids_train_5, attention_masks_train_5, labels_train)
batch_size = 4
...
input_ids = torch.cat((input_ids_train_1, input_ids_train_2, input_ids_train_3, input_ids_train_4, input_ids_train_5), 0)
input_masks = torch.cat((attention_masks_train_1, attention_masks_train_2, attention_masks_train_3, attention_masks_train_4, attention_masks_train_5),0)
...
def get_base_value():
    with torch.no_grad(): 
        ...
        model.eval() 
        inference_pair_1=["This is terrible is entailed by ."]
        inference_pair_2=["This is bad is entailed by ."]
        inference_pair_3=["This is alright is entailed by ."]
        inference_pair_4=["This is good is entailed by ."]
        inference_pair_5=["This is awesome is entailed by ."]
        ...
        torch.cuda.empty_cache()
    return output_1.logits, output_2.logits, output_3.logits, output_4.logits, output_5.logits

base_1, base_2, base_3, base_4, base_5=get_base_value()
base = torch.cat((base_1[0].reshape(1, -1), base_2[0].reshape(1, -1), base_3[0].reshape(1, -1), base_4[0].reshape(1, -1), base_5[0].reshape(1, -1)), dim=0)
print(base)
confidence_sample=[]
...
def get_eval_label():
    with torch.no_grad():
        ...
        for batch in tqdm(train_dataloader):
            b_labels = batch[10].to(device)
            b_input_ids_1 = batch[0].to(device)
            b_input_mask_1 = batch[1].to(device)
            b_input_ids_2 = batch[2].to(device)
            b_input_mask_2 = batch[3].to(device)
            b_input_ids_3 = batch[4].to(device)
            b_input_mask_3 = batch[5].to(device)
            b_input_ids_4 = batch[6].to(device)
            b_input_mask_4 = batch[7].to(device)
            b_input_ids_5 = batch[8].to(device)
            b_input_mask_5 = batch[9].to(device)
            
            b_input_ids = torch.cat((torch.unsqueeze(b_input_ids_1, 0), torch.unsqueeze(b_input_ids_2, 0),
                                     torch.unsqueeze(b_input_ids_3, 0), torch.unsqueeze(b_input_ids_4, 0),
                                     torch.unsqueeze(b_input_ids_5, 0)), dim=0)
            b_input_ids = b_input_ids.to("cpu")
            b_input_ids = torch.transpose(b_input_ids, 0, 1)
            
            b_input_mask = torch.cat((torch.unsqueeze(b_input_mask_1, 0), torch.unsqueeze(b_input_mask_2, 0),
                                      torch.unsqueeze(b_input_mask_3, 0), torch.unsqueeze(b_input_mask_4, 0),
                                      torch.unsqueeze(b_input_mask_5, 0)), dim=0)
            ...
            ans_ent=torch.cat((ans_entail_1, ans_entail_2, ans_entail_3, ans_entail_4, ans_entail_5), dim=0).reshape(5, -1).T
            ...
            ans_label = torch.cat((ans_1, ans_2, ans_3, ans_4, ans_5), dim=0).reshape(5, -1).T
            ans_label = ans_label.to("cpu").float()
            ans_label *= 2
            
            b_cls_embeddings = torch.cat((torch.unsqueeze(output_1.hidden_states[cls_layer][:, 0, :], 0), 
...
#             ans_label_cont = ans_label[:,max_cont_label]
            for i in range(len(max_entail_label)):
                ...
                cls_embeddings.append(b_cls_embeddings[i, max_entail_label[i], :].reshape(1, -1)) #roberta: -2, deberta: -6
    
            del output_1, output_2, output_3, output_4, output_5, ans_1, ans_2, ans_3, ans_4, ans_5
            torch.cuda.empty_cache()
...
confidence_unconf_once = get_unconf_nodes(eval_cls_embeddings, eval_labels.long(), k=val_k, p_list=labels_count, divisor=divisor)
...
def get_label_and_confidence():
    with torch.no_grad():
        ...
        for batch in tqdm(train_dataloader_pseudo):
            ...
            cls_embeddings.append(output.hidden_states[cls_layer][:, 0, :].to('cpu')) #roberta: -2, deberta: -6
            del output, ans
            torch.cuda.empty_cache()
...
def get_ans(num_case):
    with torch.no_grad():
        ...
        tot_confidence_list = get_unconf_nodes(tot_confidence_list, tot_label_list, k=val_k, p_list=labels_count, divisor = divisor)
        tot_confidence_list = torch.tensor(tot_confidence_list)    
        tot_confidence_list *= 2
        tot_confidence_list -=1 #-1: unconfident, 1: confident
        tot_sum = tot_confidence_list*(tot_label_list+1)
        tot_mask = tot_sum >=0
        tot_sum = tot_sum*tot_mask #every term is 0 or 1 or 3, 0 means unsure, 1 means sure that label is 0, 3 means sure that label is 2
...
torch.save(model.module.state_dict(), f'./amazon-{model_name}/finetuned_pseudo_{pth_index}.pth')
...
torch.save(model.module.state_dict(), f'./amazon-{model_name}/finetuned_confidence_{pth_index}.pth')            
...
torch.save(model.module.state_dict(), f'./amazon-{model_name}/finetuned_dropout_{pth_index}.pth')            

#dropout
torch.cuda.empty_cache()
model = train(train_unconf_dataloader, PATH)
torch.save(model.module.state_dict(), f'./amazon-{model_name}/finetuned_unconf_{pth_index}.pth')  