...
base_1, base_2, base_3, base_4=get_base_value()
base = torch.cat((base_1[0].reshape(1, -1), base_2[0].reshape(1, -1), base_3[0].reshape(1, -1), base_4[0].reshape(1, -1)), dim=0)
print(base)
confidence_sample=[]
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(train_dataloader):
            ...
            b_input_ids = torch.cat((torch.unsqueeze(b_input_ids_1, 0), torch.unsqueeze(b_input_ids_2, 0),
                                     torch.unsqueeze(b_input_ids_3, 0), torch.unsqueeze(b_input_ids_4, 0)), dim=0)
            ...
            b_input_mask = torch.cat((torch.unsqueeze(b_input_mask_1, 0), torch.unsqueeze(b_input_mask_2, 0),
                                      torch.unsqueeze(b_input_mask_3, 0), torch.unsqueeze(b_input_mask_4, 0)), dim=0)
            ...
            ans_ent=torch.cat((ans_entail_1, ans_entail_2, ans_entail_3, ans_entail_4), dim=0).reshape(4, -1).T
            max_entail_label = ans_ent.argmax(1).tolist()
            print(max_entail_label)
            

            ans_cont_1=output_1.logits[:, 2]
            ans_cont_2=output_2.logits[:, 2]
            ans_cont_3=output_3.logits[:, 2]
            ans_cont_4=output_4.logits[:, 2]
            ans_cont=torch.cat((ans_cont_1, ans_cont_2, ans_cont_3, ans_cont_4), dim=0).reshape(4, -1).T
            max_cont_label = ans_cont.argmax(1).tolist()
            ...
            ans_label = torch.cat((ans_1, ans_2, ans_3, ans_4), dim=0).reshape(4, -1).T
            ...
            b_cls_embeddings = torch.cat((torch.unsqueeze(output_1.hidden_states[cls_layer][:, 0, :], 0), 
                                          ...
                                          torch.unsqueeze(output_4.hidden_states[cls_layer][:, 0, :], 0)), dim=0)
...
#             ans_label_cont = ans_label[:,max_cont_label]
            for i in range(len(max_entail_label)):
                for j in range(4):
                    input_ids_list.append(b_input_ids[i, j, :].reshape(1, -1))
                    attention_list.append(b_input_mask[i, j, :].reshape(1, -1))
                    label_list.append(ans_label[i, j])
                    choice_list.append(j)
                    cls_embeddings.append(b_cls_embeddings[i, j, :].reshape(1, -1)) 

            del output_1, output_2, output_3, output_4, ans_1, ans_2, ans_3, ans_4
            torch.cuda.empty_cache()
...
torch.save(model.module.state_dict(), f'./ag_news-{model_name}/finetuned_pseudo_{pth_index}.pth')
...
torch.save(model.module.state_dict(), f'./ag_news-{model_name}/finetuned_confidence_{pth_index}.pth')            
...
torch.save(model.module.state_dict(), f'./ag_news-{model_name}/finetuned_dropout_{pth_index}.pth')            

#unconf
torch.cuda.empty_cache()
model = train(train_unconf_dataloader, PATH)
torch.save(model.module.state_dict(), f'./ag_news-{model_name}/finetuned_unconf_{pth_index}.pth')  
